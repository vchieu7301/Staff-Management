...
<html>
    <head>
    <link rel="stylesheet" href="styleno2.css">
    </head>
    <body>
        <?php 
        include "banner.php";
        ...
        <tr>
            <td colspan="2" align="center"><input class="btn"  type="submit" name="btn_export" value="Export" >
...
<?php
    ...
    if(isset($_POST["btn_export"])){
		...
		if (mysqli_query($conn, $sql)) {
    		...
    		$fields = array('STAFFCODE', 'SATFFNAME', 'BIRTHDAY', 'TEL1', 'TEL2', 'ZIP', 'ADDRESS1', 'ADDRESS2', 'NOTE'); 		
    		fputcsv($f, $fields);

    		while($row = mysqli_fetch_assoc($result)) {
    			$data = array($row['STAFFCODE'], $row['STAFFNAME'], $row['BIRTHDAY'], $row['TEL1'], $row['TEL2'], $row['ZIP'], $row['ADDRESS1'], $row['ADDRESS2'], $row['NOTE']);
    			fputcsv($f, $data); 
            }
            echo '<script>alert("Xuất dữ liệu thành công")</script>';
            echo "<script type='text/javascript'>window.location='export.php';</script>";
    		fclose($f); 
        }
    }
?>
